...
public interface ICommentService {
    ...
    void comment(int userId, int eventId, String commentContent);
    /**删除评论，同时删除用户评论表中的相关数据,用于普通用户的删除
     * @param commentId 取消评论人id
     * @param eventId 取消评论瓜id*/
    void cancelComment(int commentId, int eventId);
    /**删除瓜的所有评论，管理员
     * @param  eventId 瓜id*/
    void clearAll(int eventId);
    /**查看瓜的评论,也要返回评论人名
     * @param  eventId 瓜id
     * @return 所有评论信息*/
     ...
     * @return 返回第一页的评论信息
     * */
    Object[][] firstPageData(int pageSize, List<Comment> comments);
    /**进行删除，添加评论后表格刷新
     * @param comments 评论数据
     ...
     * @return 返回处理完的评论信息*/
    List<Comment> refresh(List<Comment> comments, DefaultTableModel defaultTableModel, int eventId, String[] columnNames, CommentPagingUtils commentPagingUtils);
    /**查看一条评论的信息
